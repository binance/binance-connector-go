...
func Test_binancemargintradingrestapi_AccountAPIService(t *testing.T) {
	...
	t.Run("Test AccountAPIService AdjustCrossMarginMaxLeverage Success", func(t *testing.T) {
		...
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			require.Equal(t, "3", r.URL.Query().Get("maxLeverage"))
		...
		resp, err := apiClient.RestApi.AccountAPI.AdjustCrossMarginMaxLeverage(context.Background()).MaxLeverage(int64(3)).Execute()
		require.NoError(t, err)
	...
	t.Run("Test AccountAPIService EnableIsolatedMarginAccount Success", func(t *testing.T) {
		...
		resp, err := apiClient.RestApi.AccountAPI.EnableIsolatedMarginAccount(context.Background()).Symbol("BTCUSDT").Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.Equal(
			t,
	...
	t.Run("Test AccountAPIService QueryCrossIsolatedMarginCapitalFlow Success", func(t *testing.T) {
		...
		var mockedJSON string
		mockedJSON = `[{"id":123456,"tranId":123123,"timestamp":1691116657000,"asset":"USDT","symbol":"BTCUSDT","type":"BORROW","amount":"101","note":"INSTITUTIONAL_LOAN_TRANSFER"}]`
		if mockedJSON == "" {
			mockedJSON = `{}`
		}
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	t.Run("Test AccountAPIService QueryCrossMarginFeeData Success", func(t *testing.T) {
		...
		var mockedJSON string
		mockedJSON = `[{"vipLevel":0,"coin":"BTC","transferIn":true,"borrowable":true,"dailyInterest":"0.00026125","yearlyInterest":"0.0953","borrowLimit":"180","marginablePairs":["BNBBTC"]}]`
		if mockedJSON == "" {
			mockedJSON = `{}`
		}
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	t.Run("Test AccountAPIService QueryIsolatedMarginFeeData Success", func(t *testing.T) {
		...
		var mockedJSON string
		mockedJSON = `[{"vipLevel":0,"symbol":"BTCUSDT","leverage":"10","data":[{"coin":"BTC","dailyInterest":"0.00026125","borrowLimit":"270"}]}]`
		if mockedJSON == "" {
			mockedJSON = `{}`
		}
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
