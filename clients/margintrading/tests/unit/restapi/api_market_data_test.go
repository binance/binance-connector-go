...
func Test_binancemargintradingrestapi_MarketDataAPIService(t *testing.T) {
	...
	t.Run("Test MarketDataAPIService GetMarginAssetRiskBasedLiquidationRatio Success", func(t *testing.T) {
		...
		var expected models.GetMarginAssetRiskBasedLiquidationRatioResponse
		err := json.Unmarshal([]byte(mockedJSON), &expected)
		require.NoError(t, err)
		...
		require.Equal(
			t,
			reflect.TypeOf(&common.RestApiResponse[models.GetMarginAssetRiskBasedLiquidationRatioResponse]{}),
			reflect.TypeOf(resp),
		)
		require.Equal(t, reflect.TypeOf(models.GetMarginAssetRiskBasedLiquidationRatioResponse{}), reflect.TypeOf(resp.Data))
		require.Equal(t, 200, resp.Status)
		require.Equal(t, expected, resp.Data)
	})

	t.Run("Test MarketDataAPIService GetMarginAssetRiskBasedLiquidationRatio Server Error", func(t *testing.T) {
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			http.Error(w, "internal error", http.StatusInternalServerError)
		}))
		defer mockServer.Close()
	...
	t.Run("Test MarketDataAPIService GetMarginRestrictedAssets Success", func(t *testing.T) {
		...
		var expected models.GetMarginRestrictedAssetsResponse
		err := json.Unmarshal([]byte(mockedJSON), &expected)
		require.NoError(t, err)
		...
		resp, err := apiClient.RestApi.MarketDataAPI.GetMarginRestrictedAssets(context.Background()).Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.Equal(
			t,
			reflect.TypeOf(&common.RestApiResponse[models.GetMarginRestrictedAssetsResponse]{}),
			reflect.TypeOf(resp),
		)
		require.Equal(t, reflect.TypeOf(models.GetMarginRestrictedAssetsResponse{}), reflect.TypeOf(resp.Data))
		require.Equal(t, 200, resp.Status)
		require.Equal(t, expected, resp.Data)
	})

	t.Run("Test MarketDataAPIService GetMarginRestrictedAssets Server Error", func(t *testing.T) {
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			http.Error(w, "internal error", http.StatusInternalServerError)
		}))
		defer mockServer.Close()
