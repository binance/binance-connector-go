...
With the transition to a modularized structure, the Binance Connector has been split into separate Go libraries, each focusing on a distinct product (e.g., Margin Trading, Futures, etc.). This guide explains how to migrate from the monolithic `github.com/binance/binance-connector-go` package to the new `github.com/binance/binance-connector-go/clients/margintrading` library.
...
1. **Package Name**:
   ...
   **New:** `github.com/binance/binance-connector-go/clients/margintrading`
...
3. **Imports**:
    ...
    **New:**
    ```go
    import (
        "github.com/binance/binance-connector-go/clients/margintrading"
...
4. **Configuration and Client Initialization**:  
    ...
    **New:**
    ...
    client := wallet.NewBinanceWalletClient(wallet.WithRestAPI(configuration))
    ```
...
client := margintrading.NewBinanceMarginTradingClient(margintrading.WithRestAPI(configuration))
```
...
- **Future Modular Packages**: Similar packages for other products (e.g., Spot, Sub-Account) will follow this pattern.
