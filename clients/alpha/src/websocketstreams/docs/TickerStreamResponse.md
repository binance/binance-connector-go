# TickerStreamResponse

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**E** | Pointer to **string** | eventType | [optional] 
**E** | Pointer to **int64** | eventTime | [optional] 
**S** | Pointer to **string** | symbol | [optional] 
**P** | Pointer to **string** | priceChange | [optional] 
**P** | Pointer to **string** | priceChangePercent | [optional] 
**W** | Pointer to **string** | averagePrice | [optional] 
**C** | Pointer to **string** | closePrice | [optional] 
**Q** | Pointer to **string** | lastTradeVolume | [optional] 
**O** | Pointer to **string** | openPrice | [optional] 
**H** | Pointer to **string** | highPrice | [optional] 
**L** | Pointer to **string** | lowPrice | [optional] 
**V** | Pointer to **string** | volume | [optional] 
**Q** | Pointer to **string** | quoteVolume | [optional] 
**O** | Pointer to **int64** | startTime | [optional] 
**C** | Pointer to **int64** | endTime | [optional] 
**F** | Pointer to **int64** | firstTradeId | [optional] 
**L** | Pointer to **int64** | lastTradeId | [optional] 
**N** | Pointer to **int64** | tradeNum | [optional] 

## Methods

### NewTickerStreamResponse

`func NewTickerStreamResponse() *TickerStreamResponse`

NewTickerStreamResponse instantiates a new TickerStreamResponse object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewTickerStreamResponseWithDefaults

`func NewTickerStreamResponseWithDefaults() *TickerStreamResponse`

NewTickerStreamResponseWithDefaults instantiates a new TickerStreamResponse object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetE

`func (o *TickerStreamResponse) GetE() string`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *TickerStreamResponse) GetEOk() (*string, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *TickerStreamResponse) SetE(v string)`

SetE sets E field to given value.

### HasE

`func (o *TickerStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetE

`func (o *TickerStreamResponse) GetE() int64`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *TickerStreamResponse) GetEOk() (*int64, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *TickerStreamResponse) SetE(v int64)`

SetE sets E field to given value.

### HasE

`func (o *TickerStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetS

`func (o *TickerStreamResponse) GetS() string`

GetS returns the S field if non-nil, zero value otherwise.

### GetSOk

`func (o *TickerStreamResponse) GetSOk() (*string, bool)`

GetSOk returns a tuple with the S field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetS

`func (o *TickerStreamResponse) SetS(v string)`

SetS sets S field to given value.

### HasS

`func (o *TickerStreamResponse) HasS() bool`

HasS returns a boolean if a field has been set.

### GetP

`func (o *TickerStreamResponse) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *TickerStreamResponse) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *TickerStreamResponse) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *TickerStreamResponse) HasP() bool`

HasP returns a boolean if a field has been set.

### GetP

`func (o *TickerStreamResponse) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *TickerStreamResponse) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *TickerStreamResponse) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *TickerStreamResponse) HasP() bool`

HasP returns a boolean if a field has been set.

### GetW

`func (o *TickerStreamResponse) GetW() string`

GetW returns the W field if non-nil, zero value otherwise.

### GetWOk

`func (o *TickerStreamResponse) GetWOk() (*string, bool)`

GetWOk returns a tuple with the W field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetW

`func (o *TickerStreamResponse) SetW(v string)`

SetW sets W field to given value.

### HasW

`func (o *TickerStreamResponse) HasW() bool`

HasW returns a boolean if a field has been set.

### GetC

`func (o *TickerStreamResponse) GetC() string`

GetC returns the C field if non-nil, zero value otherwise.

### GetCOk

`func (o *TickerStreamResponse) GetCOk() (*string, bool)`

GetCOk returns a tuple with the C field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetC

`func (o *TickerStreamResponse) SetC(v string)`

SetC sets C field to given value.

### HasC

`func (o *TickerStreamResponse) HasC() bool`

HasC returns a boolean if a field has been set.

### GetQ

`func (o *TickerStreamResponse) GetQ() string`

GetQ returns the Q field if non-nil, zero value otherwise.

### GetQOk

`func (o *TickerStreamResponse) GetQOk() (*string, bool)`

GetQOk returns a tuple with the Q field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetQ

`func (o *TickerStreamResponse) SetQ(v string)`

SetQ sets Q field to given value.

### HasQ

`func (o *TickerStreamResponse) HasQ() bool`

HasQ returns a boolean if a field has been set.

### GetO

`func (o *TickerStreamResponse) GetO() string`

GetO returns the O field if non-nil, zero value otherwise.

### GetOOk

`func (o *TickerStreamResponse) GetOOk() (*string, bool)`

GetOOk returns a tuple with the O field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetO

`func (o *TickerStreamResponse) SetO(v string)`

SetO sets O field to given value.

### HasO

`func (o *TickerStreamResponse) HasO() bool`

HasO returns a boolean if a field has been set.

### GetH

`func (o *TickerStreamResponse) GetH() string`

GetH returns the H field if non-nil, zero value otherwise.

### GetHOk

`func (o *TickerStreamResponse) GetHOk() (*string, bool)`

GetHOk returns a tuple with the H field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetH

`func (o *TickerStreamResponse) SetH(v string)`

SetH sets H field to given value.

### HasH

`func (o *TickerStreamResponse) HasH() bool`

HasH returns a boolean if a field has been set.

### GetL

`func (o *TickerStreamResponse) GetL() string`

GetL returns the L field if non-nil, zero value otherwise.

### GetLOk

`func (o *TickerStreamResponse) GetLOk() (*string, bool)`

GetLOk returns a tuple with the L field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetL

`func (o *TickerStreamResponse) SetL(v string)`

SetL sets L field to given value.

### HasL

`func (o *TickerStreamResponse) HasL() bool`

HasL returns a boolean if a field has been set.

### GetV

`func (o *TickerStreamResponse) GetV() string`

GetV returns the V field if non-nil, zero value otherwise.

### GetVOk

`func (o *TickerStreamResponse) GetVOk() (*string, bool)`

GetVOk returns a tuple with the V field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetV

`func (o *TickerStreamResponse) SetV(v string)`

SetV sets V field to given value.

### HasV

`func (o *TickerStreamResponse) HasV() bool`

HasV returns a boolean if a field has been set.

### GetQ

`func (o *TickerStreamResponse) GetQ() string`

GetQ returns the Q field if non-nil, zero value otherwise.

### GetQOk

`func (o *TickerStreamResponse) GetQOk() (*string, bool)`

GetQOk returns a tuple with the Q field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetQ

`func (o *TickerStreamResponse) SetQ(v string)`

SetQ sets Q field to given value.

### HasQ

`func (o *TickerStreamResponse) HasQ() bool`

HasQ returns a boolean if a field has been set.

### GetO

`func (o *TickerStreamResponse) GetO() int64`

GetO returns the O field if non-nil, zero value otherwise.

### GetOOk

`func (o *TickerStreamResponse) GetOOk() (*int64, bool)`

GetOOk returns a tuple with the O field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetO

`func (o *TickerStreamResponse) SetO(v int64)`

SetO sets O field to given value.

### HasO

`func (o *TickerStreamResponse) HasO() bool`

HasO returns a boolean if a field has been set.

### GetC

`func (o *TickerStreamResponse) GetC() int64`

GetC returns the C field if non-nil, zero value otherwise.

### GetCOk

`func (o *TickerStreamResponse) GetCOk() (*int64, bool)`

GetCOk returns a tuple with the C field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetC

`func (o *TickerStreamResponse) SetC(v int64)`

SetC sets C field to given value.

### HasC

`func (o *TickerStreamResponse) HasC() bool`

HasC returns a boolean if a field has been set.

### GetF

`func (o *TickerStreamResponse) GetF() int64`

GetF returns the F field if non-nil, zero value otherwise.

### GetFOk

`func (o *TickerStreamResponse) GetFOk() (*int64, bool)`

GetFOk returns a tuple with the F field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetF

`func (o *TickerStreamResponse) SetF(v int64)`

SetF sets F field to given value.

### HasF

`func (o *TickerStreamResponse) HasF() bool`

HasF returns a boolean if a field has been set.

### GetL

`func (o *TickerStreamResponse) GetL() int64`

GetL returns the L field if non-nil, zero value otherwise.

### GetLOk

`func (o *TickerStreamResponse) GetLOk() (*int64, bool)`

GetLOk returns a tuple with the L field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetL

`func (o *TickerStreamResponse) SetL(v int64)`

SetL sets L field to given value.

### HasL

`func (o *TickerStreamResponse) HasL() bool`

HasL returns a boolean if a field has been set.

### GetN

`func (o *TickerStreamResponse) GetN() int64`

GetN returns the N field if non-nil, zero value otherwise.

### GetNOk

`func (o *TickerStreamResponse) GetNOk() (*int64, bool)`

GetNOk returns a tuple with the N field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetN

`func (o *TickerStreamResponse) SetN(v int64)`

SetN sets N field to given value.

### HasN

`func (o *TickerStreamResponse) HasN() bool`

HasN returns a boolean if a field has been set.


[[Back to README]](../README.md)


