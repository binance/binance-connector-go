...
func AllMiniTickerStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.AllMiniTickerStream().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.AllMiniTickerStreamResponse) {
