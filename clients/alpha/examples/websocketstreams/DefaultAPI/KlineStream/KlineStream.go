...
func KlineStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.KlineStream().Symbol("alpha_116usdt").Interval(models.KlineStreamIntervalParameterInterval1m).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.KlineStreamResponse) {
