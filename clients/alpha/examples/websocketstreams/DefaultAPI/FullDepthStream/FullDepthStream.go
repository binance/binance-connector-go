...
func FullDepthStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.FullDepthStream().Symbol("alpha_116usdt").Interval(models.PartialDepthStreamIntervalParameterInterval0ms).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.FullDepthStreamResponse) {
