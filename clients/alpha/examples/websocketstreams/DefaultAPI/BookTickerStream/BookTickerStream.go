...
func BookTickerStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.BookTickerStream().Symbol("alpha_116usdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.BookTickerStreamResponse) {
