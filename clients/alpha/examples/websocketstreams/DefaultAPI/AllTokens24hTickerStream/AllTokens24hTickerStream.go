...
func AllTokens24hTickerStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.AllTokens24hTickerStream().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.AllTokens24hTickerStreamResponse) {
