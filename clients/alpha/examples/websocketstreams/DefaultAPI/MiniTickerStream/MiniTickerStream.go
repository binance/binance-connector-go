...
func MiniTickerStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.MiniTickerStream().Symbol("alpha_116usdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MiniTickerStreamResponse) {
