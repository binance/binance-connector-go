...
func AllBookTickerStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.AllBookTickerStream().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.AllBookTickerStreamResponse) {
