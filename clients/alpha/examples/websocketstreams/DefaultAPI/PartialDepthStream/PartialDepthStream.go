...
func PartialDepthStream() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.PartialDepthStream().Symbol("alpha_116usdt").Levels(models.PartialDepthStreamLevelsParameterLevels5).Interval(models.PartialDepthStreamIntervalParameterInterval0ms).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.PartialDepthStreamResponse) {
