...
// RestAPIClient manages communication with the Binance Sub Account REST API API v1.2.0
...
// NewRestAPIClient creates a new Binance Binance Sub Account REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-subaccount/1.2.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.AccountManagementAPI = &AccountManagementAPIService{client: c}
	c.ApiManagementAPI = &ApiManagementAPIService{client: c}
	c.AssetManagementAPI = &AssetManagementAPIService{client: c}
	c.ManagedSubAccountAPI = &ManagedSubAccountAPIService{client: c}
