...
func Test_binancederivativestradingcoinfuturesrestapi_UserDataStreamsAPIService(t *testing.T) {

	t.Run("Test UserDataStreamsAPIService CloseUserDataStream Success", func(t *testing.T) {

		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, "/dapi/v1/listenKey", r.URL.Path)
	...
	t.Run("Test UserDataStreamsAPIService KeepaliveUserDataStream Success", func(t *testing.T) {
		...
		mockedJSON := `{"listenKey":"vmNt6gl1so8bXVsaAY153FG5tf63QaODxUarKUM8V8rY4ElSwEe431DNIYNKOkQp"}`
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, "/dapi/v1/listenKey", r.URL.Path)
			w.Header().Set("Content-Type", "application/json")
		...
		var expected models.KeepaliveUserDataStreamResponse
		...
		resp, err := apiClient.RestApi.UserDataStreamsAPI.KeepaliveUserDataStream(context.Background()).Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.Equal(
			t,
			reflect.TypeOf(&common.RestApiResponse[models.KeepaliveUserDataStreamResponse]{}),
			reflect.TypeOf(resp),
		)
		require.Equal(t, reflect.TypeOf(models.KeepaliveUserDataStreamResponse{}), reflect.TypeOf(resp.Data))
