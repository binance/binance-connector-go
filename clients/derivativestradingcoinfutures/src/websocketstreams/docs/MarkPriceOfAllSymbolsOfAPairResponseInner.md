# MarkPriceOfAllSymbolsOfAPairResponseInner

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**E** | Pointer to **string** | Event type | [optional] 
**E** | Pointer to **int64** | Event time | [optional] 
**S** | Pointer to **string** | Symbol | [optional] 
**P** | Pointer to **string** | Mark Price | [optional] 
**P** | Pointer to **string** | Estimated Settle Price, only useful in the last hour before the settlement starts. | [optional] 
**I** | Pointer to **string** | Index Price | [optional] 
**R** | Pointer to **string** | funding rate for perpetual symbol, \&quot;\&quot; will be shown for delivery symbol | [optional] 
**T** | Pointer to **int64** | next funding time for perpetual symbol, 0 will be shown for delivery symbol | [optional] 
**St** | Pointer to **int32** | (After CM migration) Symbol type: 1 &#x3D; UM, 2 &#x3D; CM | [optional] 

## Methods

### NewMarkPriceOfAllSymbolsOfAPairResponseInner

`func NewMarkPriceOfAllSymbolsOfAPairResponseInner() *MarkPriceOfAllSymbolsOfAPairResponseInner`

NewMarkPriceOfAllSymbolsOfAPairResponseInner instantiates a new MarkPriceOfAllSymbolsOfAPairResponseInner object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewMarkPriceOfAllSymbolsOfAPairResponseInnerWithDefaults

`func NewMarkPriceOfAllSymbolsOfAPairResponseInnerWithDefaults() *MarkPriceOfAllSymbolsOfAPairResponseInner`

NewMarkPriceOfAllSymbolsOfAPairResponseInnerWithDefaults instantiates a new MarkPriceOfAllSymbolsOfAPairResponseInner object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetE() string`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetEOk() (*string, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetE(v string)`

SetE sets E field to given value.

### HasE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasE() bool`

HasE returns a boolean if a field has been set.

### GetE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetE() int64`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetEOk() (*int64, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetE(v int64)`

SetE sets E field to given value.

### HasE

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasE() bool`

HasE returns a boolean if a field has been set.

### GetS

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetS() string`

GetS returns the S field if non-nil, zero value otherwise.

### GetSOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetSOk() (*string, bool)`

GetSOk returns a tuple with the S field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetS

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetS(v string)`

SetS sets S field to given value.

### HasS

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasS() bool`

HasS returns a boolean if a field has been set.

### GetP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasP() bool`

HasP returns a boolean if a field has been set.

### GetP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasP() bool`

HasP returns a boolean if a field has been set.

### GetI

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetI() string`

GetI returns the I field if non-nil, zero value otherwise.

### GetIOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetIOk() (*string, bool)`

GetIOk returns a tuple with the I field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetI

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetI(v string)`

SetI sets I field to given value.

### HasI

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasI() bool`

HasI returns a boolean if a field has been set.

### GetR

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetR() string`

GetR returns the R field if non-nil, zero value otherwise.

### GetROk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetROk() (*string, bool)`

GetROk returns a tuple with the R field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetR

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetR(v string)`

SetR sets R field to given value.

### HasR

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasR() bool`

HasR returns a boolean if a field has been set.

### GetT

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetT() int64`

GetT returns the T field if non-nil, zero value otherwise.

### GetTOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetTOk() (*int64, bool)`

GetTOk returns a tuple with the T field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetT

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetT(v int64)`

SetT sets T field to given value.

### HasT

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasT() bool`

HasT returns a boolean if a field has been set.

### GetSt

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetSt() int32`

GetSt returns the St field if non-nil, zero value otherwise.

### GetStOk

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) GetStOk() (*int32, bool)`

GetStOk returns a tuple with the St field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSt

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) SetSt(v int32)`

SetSt sets St field to given value.

### HasSt

`func (o *MarkPriceOfAllSymbolsOfAPairResponseInner) HasSt() bool`

HasSt returns a boolean if a field has been set.


[[Back to README]](../README.md)


