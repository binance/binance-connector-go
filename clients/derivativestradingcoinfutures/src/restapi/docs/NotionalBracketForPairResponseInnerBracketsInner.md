# NotionalBracketForPairResponseInnerBracketsInner

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**Bracket** | Pointer to **int64** | bracket level | [optional] 
**InitialLeverage** | Pointer to **int64** | the maximum leverage | [optional] 
**QtyCap** | Pointer to **int64** | upper edge of base asset quantity | [optional] 
**QtylFloor** | Pointer to **int64** | lower edge of base asset quantity | [optional] 
**MaintMarginRatio** | Pointer to **float64** | Maintenance margin ratio. | [optional] 
**Cum** | Pointer to **float64** | Cumulative value. | [optional] 

## Methods

### NewNotionalBracketForPairResponseInnerBracketsInner

`func NewNotionalBracketForPairResponseInnerBracketsInner() *NotionalBracketForPairResponseInnerBracketsInner`

NewNotionalBracketForPairResponseInnerBracketsInner instantiates a new NotionalBracketForPairResponseInnerBracketsInner object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewNotionalBracketForPairResponseInnerBracketsInnerWithDefaults

`func NewNotionalBracketForPairResponseInnerBracketsInnerWithDefaults() *NotionalBracketForPairResponseInnerBracketsInner`

NewNotionalBracketForPairResponseInnerBracketsInnerWithDefaults instantiates a new NotionalBracketForPairResponseInnerBracketsInner object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetBracket

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetBracket() int64`

GetBracket returns the Bracket field if non-nil, zero value otherwise.

### GetBracketOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetBracketOk() (*int64, bool)`

GetBracketOk returns a tuple with the Bracket field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetBracket

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetBracket(v int64)`

SetBracket sets Bracket field to given value.

### HasBracket

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasBracket() bool`

HasBracket returns a boolean if a field has been set.

### GetInitialLeverage

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetInitialLeverage() int64`

GetInitialLeverage returns the InitialLeverage field if non-nil, zero value otherwise.

### GetInitialLeverageOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetInitialLeverageOk() (*int64, bool)`

GetInitialLeverageOk returns a tuple with the InitialLeverage field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetInitialLeverage

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetInitialLeverage(v int64)`

SetInitialLeverage sets InitialLeverage field to given value.

### HasInitialLeverage

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasInitialLeverage() bool`

HasInitialLeverage returns a boolean if a field has been set.

### GetQtyCap

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetQtyCap() int64`

GetQtyCap returns the QtyCap field if non-nil, zero value otherwise.

### GetQtyCapOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetQtyCapOk() (*int64, bool)`

GetQtyCapOk returns a tuple with the QtyCap field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetQtyCap

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetQtyCap(v int64)`

SetQtyCap sets QtyCap field to given value.

### HasQtyCap

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasQtyCap() bool`

HasQtyCap returns a boolean if a field has been set.

### GetQtylFloor

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetQtylFloor() int64`

GetQtylFloor returns the QtylFloor field if non-nil, zero value otherwise.

### GetQtylFloorOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetQtylFloorOk() (*int64, bool)`

GetQtylFloorOk returns a tuple with the QtylFloor field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetQtylFloor

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetQtylFloor(v int64)`

SetQtylFloor sets QtylFloor field to given value.

### HasQtylFloor

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasQtylFloor() bool`

HasQtylFloor returns a boolean if a field has been set.

### GetMaintMarginRatio

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetMaintMarginRatio() float64`

GetMaintMarginRatio returns the MaintMarginRatio field if non-nil, zero value otherwise.

### GetMaintMarginRatioOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetMaintMarginRatioOk() (*float64, bool)`

GetMaintMarginRatioOk returns a tuple with the MaintMarginRatio field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetMaintMarginRatio

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetMaintMarginRatio(v float64)`

SetMaintMarginRatio sets MaintMarginRatio field to given value.

### HasMaintMarginRatio

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasMaintMarginRatio() bool`

HasMaintMarginRatio returns a boolean if a field has been set.

### GetCum

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetCum() float64`

GetCum returns the Cum field if non-nil, zero value otherwise.

### GetCumOk

`func (o *NotionalBracketForPairResponseInnerBracketsInner) GetCumOk() (*float64, bool)`

GetCumOk returns a tuple with the Cum field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetCum

`func (o *NotionalBracketForPairResponseInnerBracketsInner) SetCum(v float64)`

SetCum sets Cum field to given value.

### HasCum

`func (o *NotionalBracketForPairResponseInnerBracketsInner) HasCum() bool`

HasCum returns a boolean if a field has been set.


[[Back to README]](../README.md)


