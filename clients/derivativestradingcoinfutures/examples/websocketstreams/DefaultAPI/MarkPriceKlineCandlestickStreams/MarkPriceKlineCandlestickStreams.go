...
func MarkPriceKlineCandlestickStreams() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.MarkPriceKlineCandlestickStreams().Symbol("btcusdt").Interval(models.ContinuousContractKlineCandlestickStreamsIntervalParameterInterval1m).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MarkPriceKlineCandlestickStreamsResponse) {
