...
func MarketLiquidationOrderStreams() {
	...
	handler, err := wsClient.WebsocketStreams.DefaultAPI.MarketLiquidationOrderStreams().Symbol("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MarketLiquidationOrderStreamsResponse) {
