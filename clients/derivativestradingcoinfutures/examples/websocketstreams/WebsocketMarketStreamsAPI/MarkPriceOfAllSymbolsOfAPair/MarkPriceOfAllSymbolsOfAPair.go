...
func MarkPriceOfAllSymbolsOfAPair() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.MarkPriceOfAllSymbolsOfAPair().Pair("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MarkPriceOfAllSymbolsOfAPairResponse) {
