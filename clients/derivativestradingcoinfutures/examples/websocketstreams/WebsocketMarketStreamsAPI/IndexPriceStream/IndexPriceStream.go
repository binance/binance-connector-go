...
func IndexPriceStream() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.IndexPriceStream().Pair("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.IndexPriceStreamResponse) {
