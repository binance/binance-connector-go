...
// RestAPIClient manages communication with the Binance Crypto Loan REST API API v1.0.0
type RestAPIClient struct {
	cfg *common.ConfigurationRestAPI

	// API Services
	FlexibleRateAPI *FlexibleRateAPIService
	StableRateAPI   *StableRateAPIService
}
...
// NewRestAPIClient creates a new Binance Binance Crypto Loan REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-cryptoloan/1.0.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.FlexibleRateAPI = &FlexibleRateAPIService{client: c}
	c.StableRateAPI = &StableRateAPIService{client: c}
