...
func Test_binanceviploanrestapi_MarketDataAPIService(t *testing.T) {
	...
	t.Run("Test MarketDataAPIService GetVIPLoanInterestRateHistory Success", func(t *testing.T) {
		...
		var expected models.GetVIPLoanInterestRateHistoryResponse
		err := json.Unmarshal([]byte(mockedJSON), &expected)
		require.NoError(t, err)
		...
		require.Equal(
			t,
			reflect.TypeOf(&common.RestApiResponse[models.GetVIPLoanInterestRateHistoryResponse]{}),
			reflect.TypeOf(resp),
		)
		require.Equal(t, reflect.TypeOf(models.GetVIPLoanInterestRateHistoryResponse{}), reflect.TypeOf(resp.Data))
		require.Equal(t, 200, resp.Status)
		require.Equal(t, expected, resp.Data)
	})

	t.Run("Test MarketDataAPIService GetVIPLoanInterestRateHistory Missing Required Params", func(t *testing.T) {
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
		defer mockServer.Close()
		...
		resp, err := apiClient.RestApi.MarketDataAPI.GetVIPLoanInterestRateHistory(context.Background()).Execute()

		require.Error(t, err)
		require.Nil(t, resp)
	})

	t.Run("Test MarketDataAPIService GetVIPLoanInterestRateHistory Server Error", func(t *testing.T) {
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			http.Error(w, "internal error", http.StatusInternalServerError)
		}))
		defer mockServer.Close()
