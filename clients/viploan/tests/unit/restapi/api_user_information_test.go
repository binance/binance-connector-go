...
func Test_binanceviploanrestapi_UserInformationAPIService(t *testing.T) {
	...
	t.Run("Test UserInformationAPIService CheckVIPLoanCollateralAccount Success", func(t *testing.T) {
		...
		mockedJSON = `{"rows":[{"collateralAccountId":"12345678","collateralCoin":"BNB,BTC,ETH"}],"total":2}`
		if mockedJSON == "" {
			mockedJSON = `{}`
		}
	...
	t.Run("Test UserInformationAPIService GetVIPLoanRepaymentHistory Success", func(t *testing.T) {
		...
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, "/sapi/v1/loan/vip/repay/history", r.URL.Path)
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(mockedJSON))
		}))
		defer mockServer.Close()

		var expected models.GetVIPLoanRepaymentHistoryResponse
		err := json.Unmarshal([]byte(mockedJSON), &expected)
		require.NoError(t, err)
		...
		resp, err := apiClient.RestApi.UserInformationAPI.GetVIPLoanRepaymentHistory(context.Background()).Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.Equal(
			t,
			reflect.TypeOf(&common.RestApiResponse[models.GetVIPLoanRepaymentHistoryResponse]{}),
			reflect.TypeOf(resp),
		)
		require.Equal(t, reflect.TypeOf(models.GetVIPLoanRepaymentHistoryResponse{}), reflect.TypeOf(resp.Data))
		require.Equal(t, 200, resp.Status)
		require.Equal(t, expected, resp.Data)
	})

	t.Run("Test UserInformationAPIService GetVIPLoanRepaymentHistory Server Error", func(t *testing.T) {
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			http.Error(w, "internal error", http.StatusInternalServerError)
		}))
		defer mockServer.Close()
