...
func OpenInterest() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.OpenInterest().UnderlyingAsset("ETH").ExpirationDate("220930").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.OpenInterestResponse) {
