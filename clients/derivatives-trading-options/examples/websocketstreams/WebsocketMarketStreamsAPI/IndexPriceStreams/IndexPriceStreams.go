...
func IndexPriceStreams() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.IndexPriceStreams().Symbol("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.IndexPriceStreamsResponse) {
