...
func Ticker24HourByUnderlyingAssetAndExpirationData() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.Ticker24HourByUnderlyingAssetAndExpirationData().UnderlyingAsset("ETH").ExpirationDate("220930").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.Ticker24HourByUnderlyingAssetAndExpirationDataResponse) {
