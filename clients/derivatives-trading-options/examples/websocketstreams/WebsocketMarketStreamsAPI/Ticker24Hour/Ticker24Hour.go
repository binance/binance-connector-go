...
func Ticker24Hour() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.Ticker24Hour().Symbol("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.Ticker24HourResponse) {
