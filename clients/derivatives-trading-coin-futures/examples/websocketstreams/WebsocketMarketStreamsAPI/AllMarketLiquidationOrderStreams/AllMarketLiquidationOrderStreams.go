...
func AllMarketLiquidationOrderStreams() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.AllMarketLiquidationOrderStreams().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.AllMarketLiquidationOrderStreamsResponse) {
