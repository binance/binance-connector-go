...
// RestAPIClient manages communication with the Binance NFT REST API API v1.3.0
...
// NewRestAPIClient creates a new Binance Binance NFT REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-nft/1.3.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.NFTAPI = &NFTAPIService{client: c}
