...
// RestAPIClient manages communication with the Binance Copy Trading REST API API v1.2.0
...
// NewRestAPIClient creates a new Binance Binance Copy Trading REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-copytrading/1.2.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.FutureCopyTradingAPI = &FutureCopyTradingAPIService{client: c}
