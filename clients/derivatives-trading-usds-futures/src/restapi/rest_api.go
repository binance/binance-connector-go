...
// RestAPIClient manages communication with the Binance Derivatives Trading USDS Futures REST API API v1.0.0
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-derivativestradingusdsfutures/1.0.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.AccountAPI = &AccountAPIService{client: c}
	c.ConvertAPI = &ConvertAPIService{client: c}
