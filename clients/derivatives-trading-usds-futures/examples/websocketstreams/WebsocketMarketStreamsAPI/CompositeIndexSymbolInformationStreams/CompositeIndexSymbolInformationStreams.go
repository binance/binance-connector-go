...
func CompositeIndexSymbolInformationStreams() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.CompositeIndexSymbolInformationStreams().Symbol("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.CompositeIndexSymbolInformationStreamsResponse) {
