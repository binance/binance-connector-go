...
func MarkPriceStreamForAllMarket() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.MarkPriceStreamForAllMarket().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MarkPriceStreamForAllMarketResponse) {
