...
func MarkPriceStream() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.MarkPriceStream().Symbol("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.MarkPriceStreamResponse) {
