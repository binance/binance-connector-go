...
func MyPreventedMatches() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.AccountAPI.MyPreventedMatches().Symbol("BNBUSDT").ExecuteAsync()
	if err != nil {
		log.Printf("Error executing MyPreventedMatches request: %v\n", err)
		return
	}
