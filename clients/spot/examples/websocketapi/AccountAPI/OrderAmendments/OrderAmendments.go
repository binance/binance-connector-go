...
func OrderAmendments() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.AccountAPI.OrderAmendments().Symbol("BNBUSDT").OrderId(1).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OrderAmendments request: %v\n", err)
		return
	}
