...
func OrderListPlaceOpoco() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.TradeAPI.OrderListPlaceOpoco().Symbol("BNBUSDT").WorkingType(models.OrderListPlaceOpoWorkingTypeParameterLimit).WorkingSide(models.OrderCancelReplaceSideParameterBuy).WorkingPrice(1).WorkingQuantity(1).PendingSide(models.OrderCancelReplaceSideParameterBuy).PendingAboveType(models.OrderListPlaceOcoAboveTypeParameterStopLossLimit).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OrderListPlaceOpoco request: %v\n", err)
		return
	}
