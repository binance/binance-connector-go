...
func OrderTest() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.TradeAPI.OrderTest().Symbol("BNBUSDT").Side(models.OrderCancelReplaceSideParameterBuy).Type(models.OrderCancelReplaceTypeParameterMarket).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OrderTest request: %v\n", err)
		return
	}
