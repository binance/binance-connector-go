...
func OrderPlace() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.TradeAPI.OrderPlace().Symbol("BNBUSDT").Side(models.OrderCancelReplaceSideParameterBuy).Type(models.OrderCancelReplaceTypeParameterMarket).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OrderPlace request: %v\n", err)
		return
	}
