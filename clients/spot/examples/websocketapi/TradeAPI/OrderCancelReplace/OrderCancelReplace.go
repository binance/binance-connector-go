...
func OrderCancelReplace() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.TradeAPI.OrderCancelReplace().Symbol("BNBUSDT").CancelReplaceMode(models.OrderCancelReplaceCancelReplaceModeParameterStopOnFailure).Side(models.OrderCancelReplaceSideParameterBuy).Type(models.OrderCancelReplaceTypeParameterMarket).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OrderCancelReplace request: %v\n", err)
		return
	}
