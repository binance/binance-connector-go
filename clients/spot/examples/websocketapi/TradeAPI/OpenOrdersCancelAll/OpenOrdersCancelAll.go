...
func OpenOrdersCancelAll() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.TradeAPI.OpenOrdersCancelAll().Symbol("BNBUSDT").ExecuteAsync()
	if err != nil {
		log.Printf("Error executing OpenOrdersCancelAll request: %v\n", err)
		return
	}
