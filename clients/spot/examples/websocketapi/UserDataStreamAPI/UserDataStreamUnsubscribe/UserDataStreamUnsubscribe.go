...
func UserDataStreamUnsubscribe() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.UserDataStreamAPI.UserDataStreamUnsubscribe().ExecuteAsync()
	if err != nil {
		log.Printf("Error executing UserDataStreamUnsubscribe request: %v\n", err)
		return
	}
