...
func UserDataStreamSubscribeSignature() {
	...
	resp, stream, errorChan, err := wsClient.WebsocketAPI.UserDataStreamAPI.UserDataStreamSubscribeSignature().Execute()
	if err != nil || errorChan != nil {
		log.Printf("Error executing UserDataStreamSubscribeSignature request: %v, %v\n", err, errorChan)
		return
	}
