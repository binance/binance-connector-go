...
func UiKlines() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.MarketAPI.UiKlines().Symbol("BNBUSDT").Interval(models.KlinesIntervalParameterInterval1s).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing UiKlines request: %v\n", err)
		return
	}
