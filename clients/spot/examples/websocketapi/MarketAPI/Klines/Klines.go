...
func Klines() {
	...
	responseChan, errorChan, err := wsClient.WebsocketAPI.MarketAPI.Klines().Symbol("BNBUSDT").Interval(models.KlinesIntervalParameterInterval1s).ExecuteAsync()
	if err != nil {
		log.Printf("Error executing Klines request: %v\n", err)
		return
	}
