...
func PartialBookDepth() {
	...
	handler, err := wsClient.WebsocketStreams.WebSocketStreamsAPI.PartialBookDepth().Symbol("bnbusdt").Levels(models.PartialBookDepthLevelsParameterLevels5).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.PartialBookDepthResponse) {
