...
func RollingWindowTicker() {
	...
	handler, err := wsClient.WebsocketStreams.WebSocketStreamsAPI.RollingWindowTicker().Symbol("bnbusdt").WindowSize(models.AllMarketRollingWindowTickerWindowSizeParameterWindowSize1h).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.RollingWindowTickerResponse) {
