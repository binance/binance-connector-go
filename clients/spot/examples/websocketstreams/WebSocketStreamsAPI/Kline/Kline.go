...
func Kline() {
	...
	handler, err := wsClient.WebsocketStreams.WebSocketStreamsAPI.Kline().Symbol("bnbusdt").Interval(models.KlineIntervalParameterInterval1s).Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.KlineResponse) {
