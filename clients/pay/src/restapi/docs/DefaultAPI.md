...
func main() {
	...
	resp, err := apiClient.RestApi.DefaultAPI.GetPayTradeHistory(context.Background()).StartTime(startTime).EndTime(endTime).Limit(limit).RecvWindow(recvWindow).Execute()
	if err != nil {
		log.Println(os.Stderr, "Error when calling `DefaultAPI.GetPayTradeHistory``: %v\n", err)
		return
	}
