...
> SignUsEquityDisclaimerResponse SignUsEquityDisclaimer(ctx).RecvWindow(recvWindow).Execute()
...
func main() {
	...
	resp, err := apiClient.RestApi.AccountAPI.SignUsEquityDisclaimer(context.Background()).RecvWindow(recvWindow).Execute()
	if err != nil {
		log.Println(os.Stderr, "Error when calling `AccountAPI.SignUsEquityDisclaimer``: %v\n", err)
		return
	}

	// response from `SignUsEquityDisclaimer`: SignUsEquityDisclaimerResponse
