...
func TradabilityStream() {
	...
	handler, err := wsClient.WebsocketStreams.MarketStreamsAPI.TradabilityStream().Symbol("AAPL").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.TradabilityStreamResponse) {
