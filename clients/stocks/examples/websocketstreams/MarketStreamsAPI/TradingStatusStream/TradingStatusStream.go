...
func TradingStatusStream() {
	...
	handler, err := wsClient.WebsocketStreams.MarketStreamsAPI.TradingStatusStream().Symbol("AAPL").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.TradingStatusStreamResponse) {
