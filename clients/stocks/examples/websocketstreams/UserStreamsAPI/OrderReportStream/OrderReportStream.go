...
func OrderReportStream() {
	...
	handler, err := wsClient.WebsocketStreams.UserStreamsAPI.OrderReportStream().ListenKey("pqia91ma19a5s61cv6a81va65sdf19v8a65a1a5s6af0dkfj2a97b8a91d").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.OrderReportStreamResponse) {
