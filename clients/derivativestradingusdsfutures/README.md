# Binance Go Derivatives Trading (USDS-M Futures) SDK
...
configuration := common.NewConfigurationRestAPI(
	common.WithBasePath(common.DerivativesTradingUsdsFuturesRestApiTestnetUrl),
)
```
...
configuration := common.NewConfigurationWebsocketApi(
	common.WithWsApiBasePath(common.DerivativesTradingUsdsFuturesWebsocketApiTestnetUrl),
)
```

If `BasePath` is not provided, it defaults to `wss://ws-fapi.binance.com/ws-fapi/v1`.
...
func UserData() {
	configuration := common.NewConfigurationWebsocketStreams(
		common.WithWsStreamsBasePath("wss://fstream.binance.com/stream"),
	)
...
configuration := common.NewConfigurationWebsocketStreams(
    common.WithWsStreamsBasePath(common.DerivativesTradingUsdsFuturesWebsocketStreamsTestnetUrl),
)
```

If `BasePath` is not provided, it defaults to `wss://fstream.binance.com`.
...
For information on migrating from previous versions of the Binance Go Derivatives Trading USDS-M Futures SDK, please refer to the [Migration Guide](./docs/migration-guide.md).
