...
func TradingSessionStream() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.TradingSessionStream().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.TradingSessionStreamResponse) {
