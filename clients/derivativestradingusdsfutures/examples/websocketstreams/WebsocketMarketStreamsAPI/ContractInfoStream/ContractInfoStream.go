...
func ContractInfoStream() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.ContractInfoStream().Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.ContractInfoStreamResponse) {
