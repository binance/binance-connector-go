...
func Test_binancederivativestradingusdsfutureswebsocketapi_TradeAPIService(t *testing.T) {
	...
	t.Run("Test TradeAPIService ModifyOrder AsyncExecute Success", func(t *testing.T) {
		...
		responseChan, errorChan, err := mockClient.WebsocketAPI.TradeAPI.ModifyOrder().Symbol("BTCUSDT").Side(models.ModifyOrderSideParameterBuy).Quantity(float64(1.0)).Price(float64(1.0)).ExecuteAsync()
		require.NoError(t, err)
	...
	t.Run("Test TradeAPIService ModifyOrder Execute Success", func(t *testing.T) {
		...
		go func() {
			resp, err := mockClient.WebsocketAPI.TradeAPI.ModifyOrder().Symbol("BTCUSDT").Side(models.ModifyOrderSideParameterBuy).Quantity(float64(1.0)).Price(float64(1.0)).Execute()
	...
	t.Run("Test TradeAPIService ModifyOrder Server Error", func(t *testing.T) {
		...
		go func() {
			respChan, _, err := mockClient.WebsocketAPI.TradeAPI.ModifyOrder().Symbol("BTCUSDT").Side(models.ModifyOrderSideParameterBuy).Quantity(float64(1.0)).Price(float64(1.0)).ExecuteAsync()
			if err != nil {
				var wsErr *common.WebSocketError
				if errors.As(err, &wsErr) {
					require.Contains(t, wsErr.Error(), "[-1001] Internal server error")
