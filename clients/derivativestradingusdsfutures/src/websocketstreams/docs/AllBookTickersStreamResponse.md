# AllBookTickersStreamResponse

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**E** | Pointer to **string** | event type | [optional] 
**U** | Pointer to **int64** | order book updateId | [optional] 
**E** | Pointer to **int64** | event time | [optional] 
**T** | Pointer to **int64** | transaction time | [optional] 
**S** | Pointer to **string** | symbol | [optional] 
**B** | Pointer to **string** | best bid price | [optional] 
**B** | Pointer to **string** | best bid qty | [optional] 
**A** | Pointer to **string** | best ask price | [optional] 
**A** | Pointer to **string** | best ask qty | [optional] 
**Ps** | Pointer to **string** | (After CM migration) Pair symbol | [optional] 
**St** | Pointer to **int32** | (After CM migration) Symbol type: 1 &#x3D; UM, 2 &#x3D; CM | [optional] 

## Methods

### NewAllBookTickersStreamResponse

`func NewAllBookTickersStreamResponse() *AllBookTickersStreamResponse`

NewAllBookTickersStreamResponse instantiates a new AllBookTickersStreamResponse object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewAllBookTickersStreamResponseWithDefaults

`func NewAllBookTickersStreamResponseWithDefaults() *AllBookTickersStreamResponse`

NewAllBookTickersStreamResponseWithDefaults instantiates a new AllBookTickersStreamResponse object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetE

`func (o *AllBookTickersStreamResponse) GetE() string`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *AllBookTickersStreamResponse) GetEOk() (*string, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *AllBookTickersStreamResponse) SetE(v string)`

SetE sets E field to given value.

### HasE

`func (o *AllBookTickersStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetU

`func (o *AllBookTickersStreamResponse) GetU() int64`

GetU returns the U field if non-nil, zero value otherwise.

### GetUOk

`func (o *AllBookTickersStreamResponse) GetUOk() (*int64, bool)`

GetUOk returns a tuple with the U field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetU

`func (o *AllBookTickersStreamResponse) SetU(v int64)`

SetU sets U field to given value.

### HasU

`func (o *AllBookTickersStreamResponse) HasU() bool`

HasU returns a boolean if a field has been set.

### GetE

`func (o *AllBookTickersStreamResponse) GetE() int64`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *AllBookTickersStreamResponse) GetEOk() (*int64, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *AllBookTickersStreamResponse) SetE(v int64)`

SetE sets E field to given value.

### HasE

`func (o *AllBookTickersStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetT

`func (o *AllBookTickersStreamResponse) GetT() int64`

GetT returns the T field if non-nil, zero value otherwise.

### GetTOk

`func (o *AllBookTickersStreamResponse) GetTOk() (*int64, bool)`

GetTOk returns a tuple with the T field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetT

`func (o *AllBookTickersStreamResponse) SetT(v int64)`

SetT sets T field to given value.

### HasT

`func (o *AllBookTickersStreamResponse) HasT() bool`

HasT returns a boolean if a field has been set.

### GetS

`func (o *AllBookTickersStreamResponse) GetS() string`

GetS returns the S field if non-nil, zero value otherwise.

### GetSOk

`func (o *AllBookTickersStreamResponse) GetSOk() (*string, bool)`

GetSOk returns a tuple with the S field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetS

`func (o *AllBookTickersStreamResponse) SetS(v string)`

SetS sets S field to given value.

### HasS

`func (o *AllBookTickersStreamResponse) HasS() bool`

HasS returns a boolean if a field has been set.

### GetB

`func (o *AllBookTickersStreamResponse) GetB() string`

GetB returns the B field if non-nil, zero value otherwise.

### GetBOk

`func (o *AllBookTickersStreamResponse) GetBOk() (*string, bool)`

GetBOk returns a tuple with the B field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetB

`func (o *AllBookTickersStreamResponse) SetB(v string)`

SetB sets B field to given value.

### HasB

`func (o *AllBookTickersStreamResponse) HasB() bool`

HasB returns a boolean if a field has been set.

### GetB

`func (o *AllBookTickersStreamResponse) GetB() string`

GetB returns the B field if non-nil, zero value otherwise.

### GetBOk

`func (o *AllBookTickersStreamResponse) GetBOk() (*string, bool)`

GetBOk returns a tuple with the B field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetB

`func (o *AllBookTickersStreamResponse) SetB(v string)`

SetB sets B field to given value.

### HasB

`func (o *AllBookTickersStreamResponse) HasB() bool`

HasB returns a boolean if a field has been set.

### GetA

`func (o *AllBookTickersStreamResponse) GetA() string`

GetA returns the A field if non-nil, zero value otherwise.

### GetAOk

`func (o *AllBookTickersStreamResponse) GetAOk() (*string, bool)`

GetAOk returns a tuple with the A field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetA

`func (o *AllBookTickersStreamResponse) SetA(v string)`

SetA sets A field to given value.

### HasA

`func (o *AllBookTickersStreamResponse) HasA() bool`

HasA returns a boolean if a field has been set.

### GetA

`func (o *AllBookTickersStreamResponse) GetA() string`

GetA returns the A field if non-nil, zero value otherwise.

### GetAOk

`func (o *AllBookTickersStreamResponse) GetAOk() (*string, bool)`

GetAOk returns a tuple with the A field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetA

`func (o *AllBookTickersStreamResponse) SetA(v string)`

SetA sets A field to given value.

### HasA

`func (o *AllBookTickersStreamResponse) HasA() bool`

HasA returns a boolean if a field has been set.

### GetPs

`func (o *AllBookTickersStreamResponse) GetPs() string`

GetPs returns the Ps field if non-nil, zero value otherwise.

### GetPsOk

`func (o *AllBookTickersStreamResponse) GetPsOk() (*string, bool)`

GetPsOk returns a tuple with the Ps field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetPs

`func (o *AllBookTickersStreamResponse) SetPs(v string)`

SetPs sets Ps field to given value.

### HasPs

`func (o *AllBookTickersStreamResponse) HasPs() bool`

HasPs returns a boolean if a field has been set.

### GetSt

`func (o *AllBookTickersStreamResponse) GetSt() int32`

GetSt returns the St field if non-nil, zero value otherwise.

### GetStOk

`func (o *AllBookTickersStreamResponse) GetStOk() (*int32, bool)`

GetStOk returns a tuple with the St field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSt

`func (o *AllBookTickersStreamResponse) SetSt(v int32)`

SetSt sets St field to given value.

### HasSt

`func (o *AllBookTickersStreamResponse) HasSt() bool`

HasSt returns a boolean if a field has been set.


[[Back to README]](../README.md)


