# MarkPriceStreamResponse

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**E** | Pointer to **string** | Event type | [optional] 
**E** | Pointer to **int64** | Event time | [optional] 
**S** | Pointer to **string** | Symbol | [optional] 
**P** | Pointer to **string** | Mark price | [optional] 
**I** | Pointer to **string** | Index price | [optional] 
**P** | Pointer to **string** | Estimated Settle Price, only useful in the last hour before the settlement starts | [optional] 
**R** | Pointer to **string** | Funding rate | [optional] 
**Ap** | Pointer to **string** | Mark price moving average | [optional] 
**T** | Pointer to **int64** | Next funding time | [optional] 
**St** | Pointer to **int32** | (After CM migration) Symbol type: 1 &#x3D; UM, 2 &#x3D; CM | [optional] 

## Methods

### NewMarkPriceStreamResponse

`func NewMarkPriceStreamResponse() *MarkPriceStreamResponse`

NewMarkPriceStreamResponse instantiates a new MarkPriceStreamResponse object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewMarkPriceStreamResponseWithDefaults

`func NewMarkPriceStreamResponseWithDefaults() *MarkPriceStreamResponse`

NewMarkPriceStreamResponseWithDefaults instantiates a new MarkPriceStreamResponse object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetE

`func (o *MarkPriceStreamResponse) GetE() string`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *MarkPriceStreamResponse) GetEOk() (*string, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *MarkPriceStreamResponse) SetE(v string)`

SetE sets E field to given value.

### HasE

`func (o *MarkPriceStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetE

`func (o *MarkPriceStreamResponse) GetE() int64`

GetE returns the E field if non-nil, zero value otherwise.

### GetEOk

`func (o *MarkPriceStreamResponse) GetEOk() (*int64, bool)`

GetEOk returns a tuple with the E field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetE

`func (o *MarkPriceStreamResponse) SetE(v int64)`

SetE sets E field to given value.

### HasE

`func (o *MarkPriceStreamResponse) HasE() bool`

HasE returns a boolean if a field has been set.

### GetS

`func (o *MarkPriceStreamResponse) GetS() string`

GetS returns the S field if non-nil, zero value otherwise.

### GetSOk

`func (o *MarkPriceStreamResponse) GetSOk() (*string, bool)`

GetSOk returns a tuple with the S field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetS

`func (o *MarkPriceStreamResponse) SetS(v string)`

SetS sets S field to given value.

### HasS

`func (o *MarkPriceStreamResponse) HasS() bool`

HasS returns a boolean if a field has been set.

### GetP

`func (o *MarkPriceStreamResponse) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *MarkPriceStreamResponse) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *MarkPriceStreamResponse) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *MarkPriceStreamResponse) HasP() bool`

HasP returns a boolean if a field has been set.

### GetI

`func (o *MarkPriceStreamResponse) GetI() string`

GetI returns the I field if non-nil, zero value otherwise.

### GetIOk

`func (o *MarkPriceStreamResponse) GetIOk() (*string, bool)`

GetIOk returns a tuple with the I field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetI

`func (o *MarkPriceStreamResponse) SetI(v string)`

SetI sets I field to given value.

### HasI

`func (o *MarkPriceStreamResponse) HasI() bool`

HasI returns a boolean if a field has been set.

### GetP

`func (o *MarkPriceStreamResponse) GetP() string`

GetP returns the P field if non-nil, zero value otherwise.

### GetPOk

`func (o *MarkPriceStreamResponse) GetPOk() (*string, bool)`

GetPOk returns a tuple with the P field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetP

`func (o *MarkPriceStreamResponse) SetP(v string)`

SetP sets P field to given value.

### HasP

`func (o *MarkPriceStreamResponse) HasP() bool`

HasP returns a boolean if a field has been set.

### GetR

`func (o *MarkPriceStreamResponse) GetR() string`

GetR returns the R field if non-nil, zero value otherwise.

### GetROk

`func (o *MarkPriceStreamResponse) GetROk() (*string, bool)`

GetROk returns a tuple with the R field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetR

`func (o *MarkPriceStreamResponse) SetR(v string)`

SetR sets R field to given value.

### HasR

`func (o *MarkPriceStreamResponse) HasR() bool`

HasR returns a boolean if a field has been set.

### GetAp

`func (o *MarkPriceStreamResponse) GetAp() string`

GetAp returns the Ap field if non-nil, zero value otherwise.

### GetApOk

`func (o *MarkPriceStreamResponse) GetApOk() (*string, bool)`

GetApOk returns a tuple with the Ap field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetAp

`func (o *MarkPriceStreamResponse) SetAp(v string)`

SetAp sets Ap field to given value.

### HasAp

`func (o *MarkPriceStreamResponse) HasAp() bool`

HasAp returns a boolean if a field has been set.

### GetT

`func (o *MarkPriceStreamResponse) GetT() int64`

GetT returns the T field if non-nil, zero value otherwise.

### GetTOk

`func (o *MarkPriceStreamResponse) GetTOk() (*int64, bool)`

GetTOk returns a tuple with the T field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetT

`func (o *MarkPriceStreamResponse) SetT(v int64)`

SetT sets T field to given value.

### HasT

`func (o *MarkPriceStreamResponse) HasT() bool`

HasT returns a boolean if a field has been set.

### GetSt

`func (o *MarkPriceStreamResponse) GetSt() int32`

GetSt returns the St field if non-nil, zero value otherwise.

### GetStOk

`func (o *MarkPriceStreamResponse) GetStOk() (*int32, bool)`

GetStOk returns a tuple with the St field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSt

`func (o *MarkPriceStreamResponse) SetSt(v int32)`

SetSt sets St field to given value.

### HasSt

`func (o *MarkPriceStreamResponse) HasSt() bool`

HasSt returns a boolean if a field has been set.


[[Back to README]](../README.md)


