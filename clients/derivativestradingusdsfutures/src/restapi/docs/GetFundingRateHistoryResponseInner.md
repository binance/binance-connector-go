# GetFundingRateHistoryResponseInner

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**Symbol** | Pointer to **string** | Symbol. | [optional] 
**FundingRate** | Pointer to **string** | Funding Rate. | [optional] 
**FundingTime** | Pointer to **int64** | Funding Time. | [optional] 
**MarkPrice** | Pointer to **string** | mark price associated with a particular funding fee charge | [optional] 
**RateType** | Pointer to **string** | Funding rate type. &#x60;Regular&#x60; for the normal funding rate; &#x60;Special&#x60; for the additional funding rate generated by stock dividends. | [optional] 

## Methods

### NewGetFundingRateHistoryResponseInner

`func NewGetFundingRateHistoryResponseInner() *GetFundingRateHistoryResponseInner`

NewGetFundingRateHistoryResponseInner instantiates a new GetFundingRateHistoryResponseInner object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewGetFundingRateHistoryResponseInnerWithDefaults

`func NewGetFundingRateHistoryResponseInnerWithDefaults() *GetFundingRateHistoryResponseInner`

NewGetFundingRateHistoryResponseInnerWithDefaults instantiates a new GetFundingRateHistoryResponseInner object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetSymbol

`func (o *GetFundingRateHistoryResponseInner) GetSymbol() string`

GetSymbol returns the Symbol field if non-nil, zero value otherwise.

### GetSymbolOk

`func (o *GetFundingRateHistoryResponseInner) GetSymbolOk() (*string, bool)`

GetSymbolOk returns a tuple with the Symbol field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSymbol

`func (o *GetFundingRateHistoryResponseInner) SetSymbol(v string)`

SetSymbol sets Symbol field to given value.

### HasSymbol

`func (o *GetFundingRateHistoryResponseInner) HasSymbol() bool`

HasSymbol returns a boolean if a field has been set.

### GetFundingRate

`func (o *GetFundingRateHistoryResponseInner) GetFundingRate() string`

GetFundingRate returns the FundingRate field if non-nil, zero value otherwise.

### GetFundingRateOk

`func (o *GetFundingRateHistoryResponseInner) GetFundingRateOk() (*string, bool)`

GetFundingRateOk returns a tuple with the FundingRate field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetFundingRate

`func (o *GetFundingRateHistoryResponseInner) SetFundingRate(v string)`

SetFundingRate sets FundingRate field to given value.

### HasFundingRate

`func (o *GetFundingRateHistoryResponseInner) HasFundingRate() bool`

HasFundingRate returns a boolean if a field has been set.

### GetFundingTime

`func (o *GetFundingRateHistoryResponseInner) GetFundingTime() int64`

GetFundingTime returns the FundingTime field if non-nil, zero value otherwise.

### GetFundingTimeOk

`func (o *GetFundingRateHistoryResponseInner) GetFundingTimeOk() (*int64, bool)`

GetFundingTimeOk returns a tuple with the FundingTime field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetFundingTime

`func (o *GetFundingRateHistoryResponseInner) SetFundingTime(v int64)`

SetFundingTime sets FundingTime field to given value.

### HasFundingTime

`func (o *GetFundingRateHistoryResponseInner) HasFundingTime() bool`

HasFundingTime returns a boolean if a field has been set.

### GetMarkPrice

`func (o *GetFundingRateHistoryResponseInner) GetMarkPrice() string`

GetMarkPrice returns the MarkPrice field if non-nil, zero value otherwise.

### GetMarkPriceOk

`func (o *GetFundingRateHistoryResponseInner) GetMarkPriceOk() (*string, bool)`

GetMarkPriceOk returns a tuple with the MarkPrice field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetMarkPrice

`func (o *GetFundingRateHistoryResponseInner) SetMarkPrice(v string)`

SetMarkPrice sets MarkPrice field to given value.

### HasMarkPrice

`func (o *GetFundingRateHistoryResponseInner) HasMarkPrice() bool`

HasMarkPrice returns a boolean if a field has been set.

### GetRateType

`func (o *GetFundingRateHistoryResponseInner) GetRateType() string`

GetRateType returns the RateType field if non-nil, zero value otherwise.

### GetRateTypeOk

`func (o *GetFundingRateHistoryResponseInner) GetRateTypeOk() (*string, bool)`

GetRateTypeOk returns a tuple with the RateType field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetRateType

`func (o *GetFundingRateHistoryResponseInner) SetRateType(v string)`

SetRateType sets RateType field to given value.

### HasRateType

`func (o *GetFundingRateHistoryResponseInner) HasRateType() bool`

HasRateType returns a boolean if a field has been set.


[[Back to README]](../README.md)


