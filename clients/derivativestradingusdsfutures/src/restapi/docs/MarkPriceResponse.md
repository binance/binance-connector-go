# MarkPriceResponse

## Properties

Name         | Type          | Description.  | Notes
------------ | ------------- | ------------- | -------------
**Symbol** | Pointer to **string** | Symbol. | [optional] 
**MarkPrice** | Pointer to **string** | mark price | [optional] 
**IndexPrice** | Pointer to **string** | index price | [optional] 
**EstimatedSettlePrice** | Pointer to **string** | Estimated Settle Price, only useful in the last hour before the settlement starts. | [optional] 
**LastFundingRate** | Pointer to **string** | This is the Latest funding rate | [optional] 
**InterestRate** | Pointer to **string** | Interest Rate. | [optional] 
**NextFundingTime** | Pointer to **int64** | Next Funding Time. | [optional] 
**Time** | Pointer to **int64** | Time. | [optional] 

## Methods

### NewMarkPriceResponse

`func NewMarkPriceResponse() *MarkPriceResponse`

NewMarkPriceResponse instantiates a new MarkPriceResponse object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewMarkPriceResponseWithDefaults

`func NewMarkPriceResponseWithDefaults() *MarkPriceResponse`

NewMarkPriceResponseWithDefaults instantiates a new MarkPriceResponse object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetSymbol

`func (o *MarkPriceResponse) GetSymbol() string`

GetSymbol returns the Symbol field if non-nil, zero value otherwise.

### GetSymbolOk

`func (o *MarkPriceResponse) GetSymbolOk() (*string, bool)`

GetSymbolOk returns a tuple with the Symbol field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSymbol

`func (o *MarkPriceResponse) SetSymbol(v string)`

SetSymbol sets Symbol field to given value.

### HasSymbol

`func (o *MarkPriceResponse) HasSymbol() bool`

HasSymbol returns a boolean if a field has been set.

### GetMarkPrice

`func (o *MarkPriceResponse) GetMarkPrice() string`

GetMarkPrice returns the MarkPrice field if non-nil, zero value otherwise.

### GetMarkPriceOk

`func (o *MarkPriceResponse) GetMarkPriceOk() (*string, bool)`

GetMarkPriceOk returns a tuple with the MarkPrice field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetMarkPrice

`func (o *MarkPriceResponse) SetMarkPrice(v string)`

SetMarkPrice sets MarkPrice field to given value.

### HasMarkPrice

`func (o *MarkPriceResponse) HasMarkPrice() bool`

HasMarkPrice returns a boolean if a field has been set.

### GetIndexPrice

`func (o *MarkPriceResponse) GetIndexPrice() string`

GetIndexPrice returns the IndexPrice field if non-nil, zero value otherwise.

### GetIndexPriceOk

`func (o *MarkPriceResponse) GetIndexPriceOk() (*string, bool)`

GetIndexPriceOk returns a tuple with the IndexPrice field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetIndexPrice

`func (o *MarkPriceResponse) SetIndexPrice(v string)`

SetIndexPrice sets IndexPrice field to given value.

### HasIndexPrice

`func (o *MarkPriceResponse) HasIndexPrice() bool`

HasIndexPrice returns a boolean if a field has been set.

### GetEstimatedSettlePrice

`func (o *MarkPriceResponse) GetEstimatedSettlePrice() string`

GetEstimatedSettlePrice returns the EstimatedSettlePrice field if non-nil, zero value otherwise.

### GetEstimatedSettlePriceOk

`func (o *MarkPriceResponse) GetEstimatedSettlePriceOk() (*string, bool)`

GetEstimatedSettlePriceOk returns a tuple with the EstimatedSettlePrice field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetEstimatedSettlePrice

`func (o *MarkPriceResponse) SetEstimatedSettlePrice(v string)`

SetEstimatedSettlePrice sets EstimatedSettlePrice field to given value.

### HasEstimatedSettlePrice

`func (o *MarkPriceResponse) HasEstimatedSettlePrice() bool`

HasEstimatedSettlePrice returns a boolean if a field has been set.

### GetLastFundingRate

`func (o *MarkPriceResponse) GetLastFundingRate() string`

GetLastFundingRate returns the LastFundingRate field if non-nil, zero value otherwise.

### GetLastFundingRateOk

`func (o *MarkPriceResponse) GetLastFundingRateOk() (*string, bool)`

GetLastFundingRateOk returns a tuple with the LastFundingRate field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetLastFundingRate

`func (o *MarkPriceResponse) SetLastFundingRate(v string)`

SetLastFundingRate sets LastFundingRate field to given value.

### HasLastFundingRate

`func (o *MarkPriceResponse) HasLastFundingRate() bool`

HasLastFundingRate returns a boolean if a field has been set.

### GetInterestRate

`func (o *MarkPriceResponse) GetInterestRate() string`

GetInterestRate returns the InterestRate field if non-nil, zero value otherwise.

### GetInterestRateOk

`func (o *MarkPriceResponse) GetInterestRateOk() (*string, bool)`

GetInterestRateOk returns a tuple with the InterestRate field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetInterestRate

`func (o *MarkPriceResponse) SetInterestRate(v string)`

SetInterestRate sets InterestRate field to given value.

### HasInterestRate

`func (o *MarkPriceResponse) HasInterestRate() bool`

HasInterestRate returns a boolean if a field has been set.

### GetNextFundingTime

`func (o *MarkPriceResponse) GetNextFundingTime() int64`

GetNextFundingTime returns the NextFundingTime field if non-nil, zero value otherwise.

### GetNextFundingTimeOk

`func (o *MarkPriceResponse) GetNextFundingTimeOk() (*int64, bool)`

GetNextFundingTimeOk returns a tuple with the NextFundingTime field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetNextFundingTime

`func (o *MarkPriceResponse) SetNextFundingTime(v int64)`

SetNextFundingTime sets NextFundingTime field to given value.

### HasNextFundingTime

`func (o *MarkPriceResponse) HasNextFundingTime() bool`

HasNextFundingTime returns a boolean if a field has been set.

### GetTime

`func (o *MarkPriceResponse) GetTime() int64`

GetTime returns the Time field if non-nil, zero value otherwise.

### GetTimeOk

`func (o *MarkPriceResponse) GetTimeOk() (*int64, bool)`

GetTimeOk returns a tuple with the Time field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetTime

`func (o *MarkPriceResponse) SetTime(v int64)`

SetTime sets Time field to given value.

### HasTime

`func (o *MarkPriceResponse) HasTime() bool`

HasTime returns a boolean if a field has been set.


[[Back to README]](../README.md)


