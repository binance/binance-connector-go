# Binance Go Derivatives Trading (Options) SDK
...
func NewSymbolInfo() {
	...
	handler, err := wsClient.WebsocketStreams.WebsocketMarketStreamsAPI.NewSymbolInfo().Execute()
	if err != nil {
...
func UserData() {
	configuration := common.NewConfigurationWebsocketStreams(
		common.WithWsStreamsBasePath("wss://nbstream.binance.com/eoptions/stream"),
	)
...
If `BasePath` is not provided, it defaults to `wss://nbstream.binance.com/eoptions`.
...
For information on migrating from previous versions of the Binance Go Derivatives Trading Options SDK, please refer to the [Migration Guide](./docs/migration-guide.md).
