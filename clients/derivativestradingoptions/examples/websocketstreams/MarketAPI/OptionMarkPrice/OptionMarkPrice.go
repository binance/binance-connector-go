...
func OptionMarkPrice() {
	...
	handler, err := wsClient.WebsocketStreams.MarketAPI.OptionMarkPrice().Underlying("btcusdt").Execute()
	if err != nil {
		log.Fatalf("Error subscribing to stream: %v", err)
	}

	handler.On("message", func(message models.OptionMarkPriceResponse) {
