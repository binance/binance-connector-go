...
func Test_binancederivativestradingoptionswebsocketstreams_MarketAPIService(t *testing.T) {
	...
	t.Run("Test MarketAPI IndexPriceStreams Success", func(t *testing.T) {
		...
		mockedJSON := `[{"e":"indexPrice","E":1763092572229,"s":"ETHUSDT","p":"3224.51976744"}]`
	...
	t.Run("Test MarketAPI OpenInterest Success", func(t *testing.T) {
		...
		resp, err := mockClient.WebsocketStreams.MarketAPI.OpenInterest().Underlying("btcusdt").ExpirationDate("220930").Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.NotEmpty(t, mockWS.MessagesWritten)
	...
	t.Run("Test MarketAPI OptionMarkPrice Success", func(t *testing.T) {
		conn, mockWS, cleanup := tests.SetupMockClient("123")
		defer cleanup()
		...
		mockedJSON := `[{"s":"BTC-251120-126000-C","mp":"770.543","E":1762867543321,"e":"markPrice","i":"104334.60217391","P":"0.000","bo":"0.000","ao":"900.000","bq":"0.0000","aq":"0.2000","b":"-1.0","a":"0.98161161","hl":"924.652","ll":"616.435","vo":"0.9408058","rf":"0.0","d":"0.11111964","t":"-164.26702615","g":"0.00001245","v":"30.63855919"}]`
		mockWS.QueueMessage([]byte(mockedJSON))

		resp, err := mockClient.WebsocketStreams.MarketAPI.OptionMarkPrice().Underlying("btcusdt").Execute()
		require.NoError(t, err)
		require.NotNil(t, resp)
		require.NotEmpty(t, mockWS.MessagesWritten)

		require.IsType(t, &common.StreamHandler[models.OptionMarkPriceResponse]{}, resp)
		require.NotNil(t, resp.On)
		require.NotNil(t, resp.Unsubscribe)

		called := 0
		var got models.OptionMarkPriceResponse
		mockCallback := func(msg models.OptionMarkPriceResponse) {
			called++
			got = msg
		}
	...
	t.Run("Test MarketAPIService OptionMarkPrice Missing Required Params", func(t *testing.T) {
		conn, _, cleanup := tests.SetupMockClient("123")
		defer cleanup()
		...
		resp, err := mockClient.WebsocketStreams.MarketAPI.OptionMarkPrice().Execute()

		require.Error(t, err)
		require.Nil(t, resp)
	})

	t.Run("Test MarketAPIService OptionMarkPrice Server Error", func(t *testing.T) {
		conn, _, cleanup := tests.SetupMockClient("123")
		defer cleanup()
