# Binance Go Derivatives Trading (Portfolio Margin Pro) SDK
...
This is a client library for the Binance Derivatives Trading Portfolio Margin Pro API, enabling developers to interact programmatically with Binance's API to suit their derivative trading needs, through two distinct endpoints:
...
```bash
go get github.com/binance/binance-connector-go/clients/derivativestradingportfoliomarginpro
```
...
WebSocket Streams is used for subscribing to user data streams. Use the [websocket-streams](./src/websocketstreams/websocket_streams.go) module to interact with it.
...
func UserData() {
	configuration := common.NewConfigurationWebsocketStreams(
		common.WithWsStreamsBasePath("wss://fstream.binance.com/pm-classic/stream"),
	)

	wsClient := client.NewBinanceDerivativesTradingPortfolioMarginProClient(
...
If `BasePath` is not provided, it defaults to `wss://fstream.binance.com/pm-classic`.
...
For information on migrating from previous versions of the Binance Go Derivatives Trading Portfolio Margin Pro SDK, please refer to the [Migration Guide](./docs/migration-guide.md).
