...
func TransferLdusdtRwusdForPortfolioMargin() {
	...
	resp, err := apiClient.RestApi.AccountAPI.TransferLdusdtRwusdForPortfolioMargin(context.Background()).Asset(models.TransferLdusdtRwusdForPortfolioMarginAssetParameterLdusdt).TransferType(models.GetTransferableEarnAssetBalanceForPortfolioMarginTransferTypeParameterEarnToFuture).Amount(1).Execute()
	if err != nil {
		log.Println(err)
		return
	}
