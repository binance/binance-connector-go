...
func TransferLdusdtRwusdForPortfolioMargin() {
	...
	resp, err := apiClient.RestApi.AccountAPI.TransferLdusdtRwusdForPortfolioMargin(context.Background()).Asset("asset_example").TransferType("transferType_example").Amount(1.0).Execute()
	if err != nil {
		log.Println(err)
		return
	}
