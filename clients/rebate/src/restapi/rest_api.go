...
// RestAPIClient manages communication with the Binance Rebate REST API API v1.1.0
...
// NewRestAPIClient creates a new Binance Binance Rebate REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-rebate/1.1.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.RebateAPI = &RebateAPIService{client: c}
