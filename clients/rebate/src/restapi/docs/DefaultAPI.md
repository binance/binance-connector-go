...
func main() {
	startTime := int64(1623319461670) // int64 | Start time in milliseconds. (optional)
	endTime := int64(1641782889000) // int64 | End time in milliseconds. (optional)
	...
	resp, err := apiClient.RestApi.DefaultAPI.GetSpotRebateHistoryRecords(context.Background()).StartTime(startTime).EndTime(endTime).Page(page).RecvWindow(recvWindow).Execute()
	if err != nil {
		log.Println(os.Stderr, "Error when calling `DefaultAPI.GetSpotRebateHistoryRecords``: %v\n", err)
		return
	}
