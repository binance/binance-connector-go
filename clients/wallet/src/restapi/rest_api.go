...
// RestAPIClient manages communication with the Binance Wallet REST API API v1.3.0
...
// NewRestAPIClient creates a new Binance Binance Wallet REST API REST API client
//
...
func NewRestAPIClient(cfg *common.ConfigurationRestAPI) *RestAPIClient {
	...
	customHeaders["User-Agent"] = "binance-wallet/1.3.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
	cfg.CustomHeaders = customHeaders
	c := &RestAPIClient{cfg: cfg}

	// API Services
	c.AccountAPI = &AccountAPIService{client: c}
	c.AssetAPI = &AssetAPIService{client: c}
	c.CapitalAPI = &CapitalAPIService{client: c}
	c.OthersAPI = &OthersAPIService{client: c}
	c.TravelRuleAPI = &TravelRuleAPIService{client: c}
