...
func GetOnChainYieldsLockedSubscriptionPreview() {
	...
	resp, err := apiClient.RestApi.OnChainYieldsAPI.GetOnChainYieldsLockedSubscriptionPreview(context.Background()).ProjectId("1").Amount(1.0).Execute()
	if err != nil {
		log.Println(err)
		return
	}
