...
func main() {
	...
	autoRepayAtCancel := true // bool | 只有在自动借款单或者自动借还单生效，true表示的是撤单后需要把订单产生的借款归还，默认为true (optional)
	recvWindow := int64(5000) // int64 |  (optional)
 ...
 **autoRepayAtCancel** | **bool** | 只有在自动借款单或者自动借还单生效，true表示的是撤单后需要把订单产生的借款归还，默认为true | 
 **recvWindow** | **int64** |  | 
