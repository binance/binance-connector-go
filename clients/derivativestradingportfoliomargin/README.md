# Binance Go Derivatives Trading (Portfolio Margin) SDK
...
This is a client library for the Binance Derivatives Trading Portfolio Margin API, enabling developers to interact programmatically with Binance's API to suit their derivative trading needs, through two distinct endpoints:
...
WebSocket Streams is used for subscribing to user data streams. Use the [websocket-streams](./src/websocketstreams/websocket_streams.go) module module to interact with it.
...
configuration := common.NewConfigurationWebsocketStreams(
    common.WithWsStreamsBasePath(common.DerivativesTradingPortfolioMarginWebsocketStreamsTestnetUrl),
)
```

If `BasePath` is not provided, it defaults to `wss://fstream.binance.com/pm`.
...
For information on migrating from previous versions of the Binance Go Derivatives Trading Portfolio Margin SDK, please refer to the [Migration Guide](./docs/migration-guide.md).
