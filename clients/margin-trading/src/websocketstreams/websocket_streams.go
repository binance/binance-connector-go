...
// WebsocketStreamsClient manages communication with the Binance Binance Margin Trading WebSocket Market Streams WebSocket Streams v1.0.0
...
func NewWebsocketStreamsClient(cfg *common.ConfigurationWebsocketStreams) *WebsocketStreamsClient {
	...
	c.userAgent = "binance-margintrading/1.0.0 (Go/" + runtime.Version() + "; " + runtime.GOOS + "; " + runtime.GOARCH + ")"
...
func (c *WebsocketStreamsClient) RiskData(listenKey string, id string) (*common.StreamHandler[models.RiskDataStreamEventsResponse], error) {
	if id != "" {
...
// @return error - An error if the operation fails
func (c *WebsocketStreamsClient) TradeData(listenKey string, id string) (*common.StreamHandler[models.TradeDataStreamEventsResponse], error) {
	if id != "" {
